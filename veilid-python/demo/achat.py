...
def getch() -> Optional[str]:
    """If a key was pressed, return it. Otherwise return None."""

    readable, _, _ = select([sys.stdin], [], [], 0)
    if not readable:
        return None
...
async def chatter(
    ...
    # Save the current state of the console. Distilled from
    # https://stackoverflow.com/questions/2408560/non-blocking-console-input
    fd = sys.stdin.fileno()
    new_term = termios.tcgetattr(fd)
    old_term = termios.tcgetattr(fd)

    # Put the console into buffered mode.
    ...
    try:
        while True:
            if (c := getch()) is not None:
                if c == "\n":
                    await router.set_dht_value(key, send_subkey, await encrypt(send_buf))
                    send_buf = ""
                    qprint(f"\n{me_prompt}")
                elif c == "\x1b":
                    print("\nClosing the chat.")
            ...
            if msg == QUIT:
                print("\nOther end closed the chat.")
                return

            print(f"\nRECV< {msg}")
            last_seq = resp.seq
            qprint(f"{me_prompt}{send_buf}")

    finally:
        # Return the console's previous settings.
